...
向建安不動產索取 `AIRE.zip`，解壓縮到任意資料夾，例如：
...
在 `AIRE/` 資料夾內找到 `.env.example`，複製並重新命名為 `.env`：
...
- **資料存放位置：** `AIRE/data/listings.db`（SQLite 資料庫，可備份）
...
**Q：資料不見了？**
確認 `AIRE/data/` 資料夾存在且有 `listings.db` 檔案。不要刪除此資料夾。
