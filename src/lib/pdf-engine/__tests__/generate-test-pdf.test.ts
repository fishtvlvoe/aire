...
import { createPdfEngine } from "../engine";
...
describe("PDF Generation E2E", () => {
  it("generates a complete land-type dossier PDF", async () => {
    ...
    const mockData: CaseDossierData = {
      caseNo: "TEST-001",
      ...
      landArea: 150.5,
      zoningType: "住宅區",
      announcedLandValue: 85000,
      legalClauses: [
        "依不動產經紀業管理條例第二十三條規定...",
        "本說明書內容如有不實，經紀業應負損害賠償責任。",
      ],
      cover: {
        propertyName: "台北市大安區忠孝東路四段100號",
        caseNumber: "TEST-001",
        handlingAgent: "王小明",
      ...
      taxCalculation: {
        landValueIncrementTax: 520000,
        landValueIncrementTaxPreferential: 130000,
        deedTax: 0,
        stampTax: 10000,
        registrationFee: 10000,
        scrivenerFee: 12000,
        totalSellerCost: 530000,
      ...
      transactionHistory: [
        ...
        { address: "復興南路一段200號", areaPing: 41.0, totalPrice: 8900000, unitPrice: 217073, transactionDate: "2025-09" },
        { address: "敦化南路一段50號", areaPing: 55.8, totalPrice: 12500000, unitPrice: 223978, transactionDate: "2025-08" },
      ],
      nearbyAmenities: [
    ...
    const blob = await engine.renderDossier({ data: mockData, themeId: "classic" });
    expect(blob).toBeDefined();
    expect(blob.size).toBeGreaterThan(1000);

    const buffer = Buffer.from(await blob.arrayBuffer());
    writeFileSync("/tmp/aire-test-land.pdf", buffer);
    console.log(`✅ Land PDF generated: /tmp/aire-test-land.pdf (${buffer.length} bytes)`);
  }, 30000);

  it("generates a complete building-type dossier PDF", async () => {
    const engine = await createPdfEngine();
    ...
    const mockData: CaseDossierData = {
      ...
      propertySheet: {
        ...
        registeredArea: 132.5,
        mainBuildingArea: 95.2,
        auxiliaryArea: 12.3,
        commonArea: 18.5,
        parkingArea: 6.5,
        floor: "12F/15F",
      ...
      taxCalculation: {
        landValueIncrementTax: 800000,
        landValueIncrementTaxPreferential: 200000,
        deedTax: 1500000,
        stampTax: 25000,
        registrationFee: 25000,
        scrivenerFee: 12000,
        totalSellerCost: 825000,
        totalBuyerCost: 1562000,
        warnings: [],
      },
      transactionHistory: [
        ...
        { address: "松仁路88號15樓", areaPing: 42.0, totalPrice: 26500000, unitPrice: 630952, transactionDate: "2025-09" },
      ],
      nearbyAmenities: [
        { name: "信義國小", category: "學校", distanceM: 400, address: "信義區松仁路" },
        { name: "台北101站", category: "捷運", distanceM: 300, address: "信義區信義路五段" },
        { name: "象山公園", category: "公園", distanceM: 800, address: "信義區信義路五段150巷" },
      ],
      surveyData: null,
      exteriorPhoto: null,
      locationMapImage: null,
    ...
    const buffer = Buffer.from(await blob.arrayBuffer());
    writeFileSync("/tmp/aire-test-building.pdf", buffer);
    console.log(`✅ Building PDF generated: /tmp/aire-test-building.pdf (${buffer.length} bytes)`);
  }, 30000);
});
