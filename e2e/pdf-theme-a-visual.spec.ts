/**
 * E2E：PDF 主題 A（淡雅 Minimal）視覺對比測試
 *
 * 情境：以 mock case data 渲染 theme-a 的封面、基本資訊頁、現況照片頁，
 *      與 mockups/pdf-themes/theme-a-minimal/ 下的 HTML mockup 做 pixel diff < 5%。
 *
 * 設計：
...
// ---------------------------------------------------------------------------
// Mock 資料：theme-a 渲染所需 case data + logo + theme settings
// ---------------------------------------------------------------------------
...
const THEME_DIR = path.resolve(__dirname, '..', 'mockups', 'pdf-themes', 'theme-a-minimal');
...
test.describe('PDF 主題 A：淡雅 Minimal — 視覺對比', () => {
  ...
  test('封面：與 mockup 視覺差異 < 5%', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('theme-a-cover.png', {
  ...
  test('基本資訊頁：與 mockup 視覺差異 < 5%', async ({ page }) => {
    ...
    // 鎖定主體內容（content.html 即基本資訊頁樣板）
    await expect(page).toHaveScreenshot('theme-a-basic-info.png', {
      maxDiffPixelRatio: MAX_DIFF_PIXEL_RATIO,
      fullPage: true,
      animations: 'disabled',
    });
  ...
  test('現況照片頁：與 mockup 視覺差異 < 5%', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('theme-a-photos.png', {
      maxDiffPixelRatio: MAX_DIFF_PIXEL_RATIO,
      fullPage: true,
      animations: 'disabled',
    });
