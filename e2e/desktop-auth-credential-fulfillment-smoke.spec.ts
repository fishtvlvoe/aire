...
test.describe("desktop auth credential fulfillment smoke", () => {
  test("bootstrap login persists device session and logout clears it", async ({ page }) => {
    ...
    const bootstrapTab = page.getByRole("tab", { name: "一次性登入碼" });
    await bootstrapTab.click();
    await expect(bootstrapTab).toHaveAttribute("aria-selected", "true");
    await page.getByPlaceholder("Email").fill("admin@test.aire");
    await page.getByPlaceholder("一次性桌面登入碼").fill("OTC-ADMIN-2026");
    await page.getByRole("button", { name: "使用桌面碼" }).click();
