...
${marketResearch}
\`\`\`
- 若 market_research 為 null 或 summary 為空字串：章節 9 周邊環境段落保持「待補」，不要臆測。
- 若有 summary：章節 9 應引用該文字（可用「依現勘人員觀察」或「業務查證」開頭）；不得改寫成自己的分析。
- 若有 attachments：章節 9 末尾以條列方式列出附件路徑，標註「相關截圖」。
...
#### 章節 6：所有權/持分
請根據 supplementary_data 中土地謄本所有權部，填入以下資料（取自 supplementary_data；若 supplementary_data 無值，改取 extracted_data，並標注（OCR讀取，請確認）；兩者皆無則待補），並撰寫 1 段簡短解讀（僅描述共有狀況，不得下法律結論）：
- 是否持分：{{待補}}
- 持分比例：{{待補}}
