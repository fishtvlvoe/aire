...
- 明確切分兩層流程：
  - 免費前查：地址候選、附近實價登錄、免費可得的公告值 / 面積 / 候選補齊
  - 付費正式查：只有 confirmed key 後，以地號 / 建號打正式 COP 或付費來源
- 明確定義 7 個欄位的資料鏈：
...
- Affected specs: `desktop-local-address-to-cop-e2e`, `registry-data-provenance`, `dossier-data-assembly`, `authorized-registry-pdf-completeness`, `real-price-query`
- Affected code:
  - Modified: `src/app/(dashboard)/cases/new/page.tsx`, `src/lib/land-registry-api.ts`, `src/lib/registry-provenance.ts`, `src/lib/pdf-engine/assemble-dossier-data.ts`, `src/lib/pdf-engine/document.tsx`, `src/components/PullParcelDataButton.tsx`, `src/app/(dashboard)/cases/[id]/preview/page.tsx`, `src/lib/export-pdf.ts`, `src/app/api/local/pdf/route.ts`
