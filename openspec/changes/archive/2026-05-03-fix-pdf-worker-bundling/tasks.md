...
- [x] 1.1 PDF OCR worker resolution survey — 讀 src/lib/ocr/ 找出 pdfjs-dist 的 import 點與 worker 設定方式 [Tool: haiku]

  輸出：列出檔案路徑、現有 import 語法、worker 是否有設 `GlobalWorkerOptions.workerSrc`。回報 ≤200 字。

### Wave 2: 修 worker 路徑

- [x] 2.1 PDF worker resolution fix — 依 design「決策」段，先實作「候選 1：改用 pdfjs-dist legacy build + URL 指向（推薦先試）」 [Tool: copilot-codex]
...
- [x] 3.1 PDF OCR end-to-end success — 跑 E2E 確認 extract 完成且 merged_fields 有值 [Tool: sonnet]
