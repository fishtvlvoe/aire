...
AIRE 的核心場景是：業務拿著紙本/PDF 謄本 + 權狀 + 現場照片，**回到電腦前要把這些資料抄進系統**，產出 5 種文件（物調表 / 591 PO 文 / 銷售 DM / 不動產說明書 / 社群貼文）。
...
### D10: 前後台 API Contract（完整端點清單）

**決策**：以下清單為本 change 所有前後台互動端點，涵蓋新建、既有修改與 cascade 行為。所有端點標準化 error payload 為 `{ error: string, code: string, detail?: object }`，HTTP status code 遵循 REST 語意。

#### 新增端點

| Method | Path | 用途 | Request | Response (200) |
|--------|------|------|---------|----------------|
| POST | `/api/listings/{id}/extract` | 觸發單一或全部 attachment OCR | query: `attachmentId?`（無則處理全部未解析）| `{ extracted: { fields, confidence }, merged_fields }` |
| GET | `/api/listings/{id}/extract-status` | Polling extract 進度 | — | `{ by_attachment: { [attId]: { status: "pending"\|"parsing"\|"done"\|"failed", error?: string, completed_at?: ISO } } }` |
| POST | `/api/listings/{id}/field-visit/switch-source` | 切換 merged_fields 某欄位的來源 | body: `{ fieldKey: string, attachmentId: string }` | `{ merged_fields }` |

#### 既有端點修改（behavior change）

| Method | Path | 行為變更 |
|--------|------|---------|
| POST | `/api/listings/{id}/attachments` | 上傳成功後 SHALL 以 fire-and-forget 觸發 `POST /extract?attachmentId={id}`；回傳仍為 `{ attachment }`（不等 OCR）|
| DELETE | `/api/listings/{id}/attachments` | 刪除 attachment 時 SHALL cascade 清除 `extracted_data.by_attachment.<attId>` 並**重新計算** `merged_fields`（保留其他來源提供的欄位）|
| GET | `/api/listings/{id}` | Response payload SHALL 含完整 `listing.extracted_data`（若 non-null），前端讀 `extracted_data.merged_fields` 作為 FieldVisitForm 的 initialData 補強來源 |
| POST | `/api/listings/{id}/field-visit` | 接收 body.fields 時，若某 fieldKey 原 provenance 為 `ocr-pdf` / `ocr-image` / `llm-vision`，且新值與 `merged_fields.<key>.value` 不同，SHALL 標記為 `manual-edit`（前端 UI 轉橘色徽章依據此標記）|
...
```json
{
  ...
  "detail": { /* optional, context-specific */ }
}
```
...
|-------------|---------|
| 200 | 操作成功 |
| 202 | Extract 已接受但非同步處理中（extract endpoint 非同步模式）|
| 400 | Request body 格式錯誤、missing required field |
| 404 | listing / attachment 不存在 |
| 409 | field_visit 衝突（optional 版本控制）|
| 500 | OCR pipeline 內部錯誤（Layer 1/2 全失敗，且未 opt-in Layer 3）|

**理由**：前端 (FieldVisitForm / PhotoUploadClassifier / SupplementaryForm) 對「OCR 解析中」「切換來源」「刪檔連動」「業務手改變橘色徽章」4 個 UX 行為都需明確後端 API 支援。沒有此合約，Copilot 施工時會各自猜測端點與 payload，導致前後台脫節。
