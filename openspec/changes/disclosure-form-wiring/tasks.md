...
- [ ] [P] 3.1 修改 `src/components/disclosure-form-residential.tsx` props 介面，新增 `initialPayload?: Record<string, unknown>` 和 `onChange?: (payload: Record<string, unknown>) => void`（需求：Integration with CaseWizard Step 3）；mount 時用 `initialPayload` 填充所有欄位初始值；每個欄位 onChange 時呼叫外部 `onChange` 並傳入完整 payload（包含所有已填欄位）。不得移除元件現有的 standalone 使用能力（props 為 optional）。驗收：傳入 `initialPayload = { building_area: "50" }` 時，建物面積欄位顯示 "50"；修改任一欄位後 `onChange` 被呼叫且 payload 包含所有欄位的當前值
- [ ] [P] 3.2 同上，修改 `src/components/disclosure-form-land.tsx` 加入相同 `initialPayload` 和 `onChange` props 介面（需求：Integration with CaseWizard Step 3）。驗收：傳入 `initialPayload = { land_area: "100" }` 時，土地面積欄位顯示 "100"；修改任一欄位後 `onChange` 被呼叫
