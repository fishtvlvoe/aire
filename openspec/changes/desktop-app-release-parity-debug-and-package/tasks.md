...
- [ ] 4.2 覆蓋 Decision: Release parity evidence is platform-specific 與 Requirement: Desktop release parity SHALL include runtime evidence per platform；在 macOS Tauri App 跑地址查詢、建立案件與正式匯入 gate smoke，保存 App 截圖與 log。
- [ ] 4.3 覆蓋 Decision: Release parity evidence is platform-specific 與 Requirement: Desktop release parity SHALL include runtime evidence per platform；產出 Windows installer 或選定 GitHub Windows build，保存 commit SHA、installer checksum、runtime smoke report 與限制說明。
