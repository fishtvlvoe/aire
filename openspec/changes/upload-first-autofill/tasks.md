...
- [ ] 1.4 規則 parser — 土地謄本：`src/lib/ocr/parsers/transcript-land.ts`。regex 抽欄位：地段、地號、面積（平方公尺 → 坪換算）、公告地價、公告現值、所有權人、他項權利、權利範圍。[Tool: copilot-codex]
- [ ] 1.5 規則 parser — 建物謄本：`src/lib/ocr/parsers/transcript-building.ts`。欄位：建號、坐落地址、建物面積、主要用途、層數、結構、建築完成日。[Tool: copilot-codex]
- [ ] 1.6 單元測試：對 parser 餵入 5+ 份真實謄本 raw text（脫敏），驗證欄位抽出正確。[Tool: copilot-codex]
