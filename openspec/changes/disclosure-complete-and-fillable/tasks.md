...
- [ ] 6.1 新增 `src/lib/document-generator/__tests__/acroform-overlay.test.ts`：用 `pdf-lib` 建立最小 PDF，呼叫 `overlayAcroForm` 傳入 2 個欄位的 coordMap，再用 `pdf-lib` 讀取結果 PDF 確認 AcroForm 欄位數量 = 2、fieldName 正確。[Tool: sonnet]
