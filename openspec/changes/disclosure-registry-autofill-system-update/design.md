...
- 案件/章節區採地址先行：先用地址、門牌、地號或建號進行地政判斷，自動判斷土地/建物組成與物件類型；只有查不到、候選多筆或使用者從非地址起案時才顯示人工選擇。
...
- 左側一級選單需提供可見的收合控制；底部固定顯示客戶頭像與個人設定入口，不放方案說明或教學提示卡。
- 章節選擇優先用短標籤或自動導引，不用一開始就顯示冗長下拉；系統依地政判斷結果把使用者帶到需要審核的章節。
...
- 1440px：完整兩欄工作台，左側一級選單有收合按鈕與個人設定入口。
- 1024px：案件/章節與欄位審核可讀。
- 768px：主要表單不被遮住。
