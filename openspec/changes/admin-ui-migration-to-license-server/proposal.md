...
three-ai 主 App 是 Electron 桌面版（用 better-sqlite3 本機資料庫），admin UI（src/app/admin/licenses/page.tsx，840 行 React）只能在 Fish 自己電腦上跑；當 Fish 不在電腦前需要核發、停用或重發序號（例如客戶突然在外打電話來），無法處理。把 admin UI 搬到已上線的 license-server（Vercel + KV），就能從手機或任何瀏覽器即時管理序號。

## What Changes

- 新增 license-server 改造為 Next.js 14 App Router 專案（保留現有 Vercel Functions 風格 API 不變更，採取漸進整合）
- 新增 `app/admin/licenses/page.tsx` — 從 three-ai 移植過來的 admin UI（840 行 React，調整 API endpoint 與 auth 機制）
...
- 新增 admin proxy API 端點（`app/api/admin/licenses/route.ts` 等）— 把原 three-ai 的 `/api/admin/licenses*` proxy 路由搬到 license-server，內部呼叫現有 `lib/store.ts`
...
- 新增 `next.config.ts` 與 `tsconfig.json`（沿用 three-ai 慣例）
...
- 不刪除 three-ai 端的 admin UI（`src/app/admin/licenses/page.tsx` 保留給本機用）
