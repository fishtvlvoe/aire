# three-ai SDD 實作 Prompt

你是 three-ai 專案的實作代理。以下是 3 個 Spectra Change 的完整規格（proposal + design + tasks），請按照 tasks.md 中的任務清單逐一實作。
...
新檔案 `src/lib/auth/vendor.ts` 匯出 `provisionVendorAccount(credentials)`。流程：查詢 users 表是否已有同 username 且 is_vendor = 1 的紀錄。存在則 UPDATE password_hash 和 display_name；不存在則 INSERT 新紀錄，role 固定為 admin，is_vendor = 1。email 使用 `{username}@vendor.three-ai.app` 格式（內部用，不寄信）。密碼 hash 由 License Server 預先計算（bcrypt），客戶端直接存入 DB。
...
- **取捨**：vendor 帳號的 email 是虛擬格式（@vendor.three-ai.app），不支援密碼重設。系統商若忘記密碼需重新驗證 license。
