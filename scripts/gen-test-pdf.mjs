...
console.log("Generating land PDF...");
try {
  const blob = await engine.renderDossier({ data: landData, themeId: "classic" });
  const buf = Buffer.from(await blob.arrayBuffer());
  writeFileSync("/tmp/aire-test-land.pdf", buf);
  console.log(`✅ Land PDF: /tmp/aire-test-land.pdf (${buf.length} bytes)`);
} catch (e) {
  console.error("❌ Land PDF failed:", e.message);
  if (e.cause) console.error("   Cause:", e.cause.message);
}

console.log("Generating building PDF...");
try {
  const blob = await engine.renderDossier({ data: buildingData, themeId: "classic" });
  const buf = Buffer.from(await blob.arrayBuffer());
  writeFileSync("/tmp/aire-test-building.pdf", buf);
  console.log(`✅ Building PDF: /tmp/aire-test-building.pdf (${buf.length} bytes)`);
} catch (e) {
  console.error("❌ Building PDF failed:", e.message);
  if (e.cause) console.error("   Cause:", e.cause.message);
}
